...
def data_return(request):
	if request.method == 'POST':
		...
		# Database
		filename = register.objects.filter(ACCOUNT_NUMBER=X[1]).values('SIGNATURE_URLS')
		xx = filename[0]['SIGNATURE_URLS']
		...
		probability = signet.signet_classifier('Images/SignImage.png', 'media/Signatures/' + xx)
		print (probability)
		#x = "{% static '" + filename[0] + "' %}"
